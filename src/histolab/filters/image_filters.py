...
class LocalOtsuThreshold(ImageFilter):
    ...
    Compute local Otsu threshold for each pixel within a neighborhood defined by a disk
    structuring element (with radius 3 by default) and return a boolean mask based on
    pixels being less than the local Otsu threshold. This procedure improves
    segmentation on images with an uneven illumination [5]_.
    ...
    References
    ----------
    .. [5] Y Liu and S.N. Srihari. “Document image binarization based on texture
        features”. IEEE Trans Pattern Anal Mach Intell 19.5 (1997)
    """
